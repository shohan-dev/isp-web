...
<script>
  $(document).ready(function() {
    const loadTraffic = () => {
      ...
      // Dynamically update the URL with the router ID when the page loads
      let baseUrl = "<?= base_url('/routers/load-traffic'); ?>";
      let url = `${baseUrl}/${routerId}`;


      // Update the href attribute of the "View Details" link
      // $('#viewDetailsLink').attr('href', newUrl);

      // url: `<?= route_to('route.routers.load_traffic', 11); ?>`,
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          const activeUsers = result.data.activeusers || []; // Fallback to empty array
