...
IP_ARG    = sys.argv[2] if len(sys.argv) > 2 else "103.115.158.16"
PORT_ARG  = sys.argv[3] if len(sys.argv) > 3 else "8003"
USERNAME  = sys.argv[4] if len(sys.argv) > 4 else "admin"
PASSWORD  = sys.argv[5] if len(sys.argv) > 5 else "Xpon@Olt9417#"
...
# ========= LOGIN FUNCTIONS =========
def vsol_get_session(ip, username, password):
    """Login to VSOL OLT and get session"""
    try:
...
def vsol_login(ip, username, password):
    """Alternative login function for VSOL"""
    try:
        url = f"https://{ip}/action/main.html"
...
# ========= VSOL EPON =========
def vsol_epon(olt, ip, username, password, action, pon_ports, key, cmd, onuid):
    """For VSOL EPON OLTs"""
    try:
        ...
        if body_tag and not body_tag.text.strip():
            login_result = vsol_get_session(ip, username, password)
            if not login_result:
...
# ========= VSOL GPON =========
def vsol_gpon(olt, ip, username, password, action, pon_ports, key, cmd, onuid):
    """For VSOL GPON OLTs"""
    try:
        current_date_time = now()
        ...
        if body_tag and not body_tag.text.strip():
            login_result = vsol_login(ip, username, password)
            if not login_result:
                return error_response(action, olt)
...
# ========= AUTO-DETECT VSOL TYPE =========
def vsol_auto(olt, ip, username, password, action, pon_ports, key, cmd, onuid):
    """Auto-detect VSOL OLT type and use appropriate function"""
    try:
        
        # First try EPON
        try:
            result = vsol_epon(olt, ip, username, password, action, pon_ports, key, cmd, onuid)
            if result and "ERROR" not in result:
                print("Detected VSOL EPON", file=sys.stderr)
                return result
        except:
            pass
        ...
        # Then try GPON
        try:
            result = vsol_gpon(olt, ip, username, password, action, pon_ports, key, cmd, onuid)
            if result and "ERROR" not in result:
                print("Detected VSOL GPON", file=sys.stderr)
                return result
        except:
            pass
