...
<div class="box box-warning">
            ...
            <?php
              ob_start();
              if (userHasPermission('inventory_purchess', 'create')): ?>

                        <a class="btn btn-primary" href="#" data-toggle="modal" data-target="#billModal">
                    ...
                    <?php endif;
              $purchessActionsHtml = ob_get_clean();
            ?>
              ...
              <?= view('components/list-toolbar', [
                'filters' => [
                  [
                    ...
                    'type' => 'select',
                    'ariaLabel' => 'Providers',
                    'emptyLabel' => 'Select Providers',
                    'options' => array_map(static function ($reseller) {
                        return ['value' => $reseller['id'], 'label' => $reseller['company_name']];
                    }, $providers),
                  ],
                  [
                    'id' => 'status',
                ...
                'actionsHtml' => $purchessActionsHtml,
                'manualBind' => true,
              ]); ?>
            </div>
            <div class="box-header with-border">
                <form id="filterForm" method="post">
                    <div class="ipb-filter-actions" style="margin-top: 0; display:flex; flex-wrap:wrap; gap:8px; justify-content:flex-end;">
            ...
            <div class="box-body">
                ...
                <table class="table table-bordered table-striped datatable">
                    <caption class="sr-only">Purchase bill history</caption>
                    ...
                    <?php
                      ...
                      $purchessSkeletonCols = 10
                        + (userHasPermission('inventory_purchess', 'delete') ? 1 : 0)
                        + (userHasPermission('inventory_purchess', 'update') ? 1 : 0);
                    ?>
                    <?= view('components/skeleton-table', ['cols' => $purchessSkeletonCols, 'rows' => 8]) ?>
...
<div class="modal fade" id="billModal" tabindex="-1" role="dialog" aria-labelledby="billModalLabel">
    <div class="modal-dialog modal-xl" role="document" style="max-width:min(1100px, calc(100vw - 16px)); width:auto; margin:12px auto;">
...
<script>
    $(document).ready(function () {
        ...
        <tr class="item-row">
            ...
            <td><button type="button" class="btn btn-danger btn-sm remove-row" aria-label="Remove item"><i class="fa fa-times" aria-hidden="true"></i></button></td>
        </tr>`;
    ...
    <tr class="item-row">
        ...
        if (window.IpbFilters) {
            IpbFilters.restore({ storageKey: 'ipb_filters_inventory_purchess', root: '#filterForm' });
        }
        ...
        // 04 §5/§10 — wire the shared IpbFilters loading feedback (preXhr/xhr/error ->
        // .is-loading dim+skeleton overlay on this table), matching bandwidth/purchess.php.
        var ipbInvPurchessFilters = null;
        if (window.IpbFilters) {
            ipbInvPurchessFilters = IpbFilters.bind(table, {
                storageKey: 'ipb_filters_inventory_purchess',
                root: '#filterForm',
        ...
        $('#applyFilter').click(function () {
            table.ajax.reload();
            if (ipbInvPurchessFilters) ipbInvPurchessFilters.updateBadge();
        });
