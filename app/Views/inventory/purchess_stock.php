<?php
function findStockDetails($itemSummary, $units, $itemName, $categoryId, $subcategoryId)
{
    foreach ($itemSummary as $entry) {
        if (
            $entry['item_name'] === $itemName &&
            $entry['category'] == $categoryId &&
            $entry['subcategory'] == $subcategoryId
        ) {
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        <caption class="sr-only">Purchase stock items</caption>
                        <thead>
                            <tr>
                                    ...
                                    ?>
                                                <tr data-parent="<?= $category['id'] ?>" class="category-item" style="display:none;">
                                                    ...
                                                    <td style="padding-left: 35px; margin-top: -20px; ">
                                                        <span
                                                            style=" margin-right: 5px; margin-left: 2px; border-left: 2px solid var(--border-strong); padding-top: 18px;">---</span>
                                                    ...
                                                    <?php $details = findStockDetails($purchess_stock, $units, $catItem['item_name'], $category['id'], 0); ?>
                                                    <td>
                                                        <?= htmlspecialchars($details['total_qty']) ?> <?= htmlspecialchars($details['unit_name']) ?>
                                                    </td>
                                            ...
                                            <?php foreach ($subItems as $index => $subItem): ?>
                                                <?php if (isset($subItem['item_name'], $subItem['unit'], $subItem['vat'])): ?>
                                                    <tr data-subcat-parent="<?= htmlspecialchars($subcategory['subcategory_name']) ?>"
                                                        ...
                                                        <?php $details = findStockDetails($purchess_stock, $units, $subItem['item_name'], $category['id'], $index); ?>
                                                        <td>
                                                            <?= htmlspecialchars($details['total_qty']) ?> <?= htmlspecialchars($details['unit_name']) ?>
                                                        </td>
