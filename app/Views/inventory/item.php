...
<?php if (userHasPermission('inventory_purchess', 'create')) : ?>
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped ipb-cards-sm">
                        <caption class="sr-only">Inventory items</caption>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <?php foreach ($parent_categories as $category): ?>
                                ...
                                <tr data-id="<?= $category['id'] ?>" class="category-parent">
                                    <?php if (userHasPermission('inventory_purchess', 'delete')): ?>
                                        <td data-label="<?= esc('Select') ?>"><input type="checkbox" class="input-check-selected" value="<?= $category['id'] ?>">
                                        </td>
                                    <?php endif; ?>
                                    <td data-label="<?= esc('Category / Subcategory / Item Name') ?>">
                                    ...
                                    <td data-label="<?= esc('Unit Price') ?>"></td>
                                    <td data-label="<?= esc('% Vat') ?>"></td>
                                    <!-- <td>
                                        ...
                                        foreach ($catItems as $mindex => $catItem):
                                            if (isset($catItem['item_name']) && isset($catItem['unit']) && isset($catItem['vat'])):
                                                ...
                                                <tr data-parent="<?= $category['id'] ?>" class="category-item" style="display:none;">
                                                    <td data-label="<?= esc('Select') ?>"></td>

                                                    <td data-label="<?= esc('Category / Subcategory / Item Name') ?>" style="padding-left: 35px; margin-top: -20px; ">
                                                        <span
                                                    ...
                                                    <td data-label="<?= esc('Unit Price') ?>"><?= htmlspecialchars($catItem['unit']) ?></td>
                                                    <td data-label="<?= esc('% Vat') ?>"><?= htmlspecialchars($catItem['vat']) ?></td>
                                                    <td class="ipb-cards-actions" data-label="<?= esc('Action') ?>">
                                                        ...
                                                        <?php endif; ?>
                                                        <?php if (userHasPermission('inventory_purchess', 'delete')) : ?>
                                                        <button class="btn btn-xs btn-danger delete-item" data-item='<?= htmlspecialchars(json_encode([
                                                            'item_name' => $catItem['item_name'],
                                                            'unit' => $catItem['unit']
                                                        ]), ENT_QUOTES, 'UTF-8') ?>'
                                                            data-category-id="<?= $category['id'] ?>" data-item-index="<?= $mindex ?>"
                                                            data-item-type="category" title="Delete this item">
                                ...
                                <?php if (!empty($subcategory_items) && is_array($subcategory_items)): ?>
                                    <?php foreach ($subcategory_items as $subcategoryindex => $subcategory): ?>
                                        <tr data-parent="<?= htmlspecialchars($category['id']) ?>"
                                            ...
                                            <td data-label="<?= esc('Select') ?>"></td>
                                            <td data-label="<?= esc('Category / Subcategory / Item Name') ?>" style="padding-left: 25px;">
                                            ...
                                            <?php foreach ($subItems as $index => $subItem): ?>
                                                <?php if (isset($subItem['item_name'], $subItem['unit'], $subItem['vat'])): ?>
                                                    <tr data-subcat-parent="<?= htmlspecialchars($subcategory['subcategory_name']) ?>"
                                                        ...
                                                        <td data-label="<?= esc('Select') ?>"></td>
                                                        <td data-label="<?= esc('Category / Subcategory / Item Name') ?>" style="padding-left: 53px; margin-top: -20px; ">
                                                            <span
                                                                style=" margin-right: 5px; margin-left: 2px; border-left: 2px solid black; padding-top: 18px;">---</span>
                                                        ...
                                                        <td data-label="<?= esc('Unit Price') ?>"><?= htmlspecialchars($subItem['unit']) ?></td>
                                                        <td data-label="<?= esc('% Vat') ?>"><?= htmlspecialchars($subItem['vat']) ?></td>
                                                        <td class="ipb-cards-actions" data-label="<?= esc('Action') ?>">
                                                            <?php if (userHasPermission('inventory_purchess', 'update')) : ?>
                                                            <button class="btn btn-xs btn-primary edit-item"
    ...
    <div class="modal fade" id="addItemModal" tabindex="-1" role="dialog" aria-labelledby="addItemModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="addItemForm" action="<?= route_to('inventory.item_store'); ?>" method="POST">
                    <?= csrf_field() ?>
...
<script>
    $(document).on('click', '.edit-item', function () {
        try {
            ...
            $('#addItemForm').attr('action', '<?= route_to("inventory.item_update") ?>');
    ...
    $('#addItemModal').on('hidden.bs.modal', function () {
        $('#addItemForm')[0].reset();
        $('#addItemForm').attr('action', '<?= route_to("inventory.item_store"); ?>');
        $('#addItemForm').find('[name="item_index"], [name="subcategory_index"], [name="subcategory_name"]').remove();
    ...
    swal({
        ...
        if (willDelete) {
            ...
            $.ajax({
                url: '<?= route_to("inventory.item_delete") ?>',
                type: 'POST',
                data: {
        ...
        $('#category').change(function () {
            ...
            if (categoryId) {
                $.ajax({
                    url: '<?= route_to("inventory.getSubcategories"); ?>',
                    type: 'GET',
