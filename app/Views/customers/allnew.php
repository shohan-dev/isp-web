      ...
      <div class="box-body">
        <div class="table-responsive ipb-customers-table-wrap">
          <table class="table table-bordered table-striped datatable">
            <caption class="sr-only">New customer list</caption>
            <thead class="text-nowrap">
              <tr>
                <?php if (userHasPermission('customer', 'delete')): ?>
                  <th scope="col" data-data="select" data-col="select" data-col-locked="1"><input type="checkbox" class="form-check-input" id="select_all" aria-label="Select all"></th>
                <?php endif; ?>
            ...
            <?php
              ...
              $customersNewSkeletonCols = 13 + (userHasPermission('customer', 'delete') ? 1 : 0);
            ?>
            <?= view('components/skeleton-table', ['cols' => $customersNewSkeletonCols, 'rows' => 8]) ?>
          </table>
        </div>
      </div>
    </div>
...
<script>
  ...
  $(document).ready(function () {
    ...
    const table = $('.datatable').DataTable({
      ...
      ajax: {
        url: '<?= route_to("route.customer.new_fetch"); ?>',
        type: 'POST',
