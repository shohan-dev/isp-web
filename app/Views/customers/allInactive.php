      ...
      <div class="box-body">
        <div class="table-responsive ipb-customers-table-wrap">
          <table class="table table-bordered table-striped datatable">
            <caption class="sr-only">Inactive customers</caption>
            ...
            <?php
              ...
              $inactiveCustomersSkeletonCols = 13 + (userHasPermission('customer', 'delete') ? 1 : 0);
            ?>
            <?= view('components/skeleton-table', ['cols' => $inactiveCustomersSkeletonCols, 'rows' => 8]) ?>
          </table>
        </div>
      </div>
    </div>
...
<script>
  ...
  $(document).ready(function () {
    ...
    const table = $('.datatable').DataTable({
      ...
      ajax: {
        url: '<?= route_to("route.customer.inactive_fetch"); ?>',
        type: 'POST',
