        ...
        <div class="row">
          ...
          <div class="col-xs-6">
            <div class="form-group">
              ...
              echo form_dropdown('pricing_type', $options, '', 'class="form-control"'); ?>
...
<script>
  $("#form").submit(function(e) {
    ...
    $.ajax({
      url: '<?= route_to('route.packages.create'); ?>',
      type: 'POST',
      ...
      success: function(result) {
        ...
        $(form).find('button[type="submit"]').html('Add Package');

        $(form).find('button[type="submit"]').removeAttr('disabled');

        $(form).trigger('reset');

        tata.success('Package added', result.response, {
          onClose: () => {
      ...
      error: function({
        ...
        if (result.status === 'validation-error') {
          ...
          tata.error("Couldn't add package", result.response);
        }
      }
    });
