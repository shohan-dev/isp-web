...
<script>
  $("#form").submit(function (e) {
    ...
    $.ajax({
      url: '<?= route_to('route.subarea.create'); ?>',
      type: 'POST',
      data: function () {
        let formData = new FormData(form);
        formData.append('id', <?= $id ?? 'null' ?>); // Adding the ID to the form data
        return formData;
      }(),
      contentType: false,
      cache: false,
      ...
      success: function (result) {
        ...
        tata.success('Sub-area added', result.response, {
          onClose: () => {
            window.history.back();
          },
        });
      ...
      error: function ({ responseText }) {
        ...
        if (result.status === 'validation-error') {
          ...
          tata.error("Couldn't add sub-area", result.response);
        }
      }
    });
