...
{
    protected $bandwidthModel, $BandwidthBuyCategory, $providerModel, $puchessModel ,$vendorModel;

    public function __construct()
    {
        // Load the BandwidthModel
        $this->bandwidthModel = new BandwidthModel();
        $this->BandwidthBuyCategory = new BandwidthBuyCategory();
    ...
    // Method to list all bandwidth packages
    public function index()
    {
        /* This used to render bandwidth/index.php, which is an unmodified copy
           of the reseller list: its DataTable loads rows from
           route.Reseller.fetch and its "Delete Item" button posts to
           route.reseller.delete, while the page is labelled "Item" and was
           gated on the unrelated profile_update:update permission. So /bandwidth
           exposed the reseller list and let a user delete resellers from a page
           claiming to manage bandwidth items — and it ignored the $bandwidths
           this method passed it. Nothing links here (the sidebar's Bandwidth
           menu points at bandwidth.item_index), so land on the real bandwidth
           item list, which enforces its own permission check. */
        return redirect()->route('bandwidth.item_index');
    }
    public function category_index()
    {
        if (!userHasPermission('profile_update', 'update'))
            show_404();
        $data['parent_categories'] = $this->BandwidthBuyCategory->getAllBandwidthPackages();
        return view('bandwidth/item_category', $data); // Create a corresponding view
    }
    public function item_index()
    {
        if (!userHasPermission('profile_update', 'update'))
            show_404();
        $data['parent_categories'] = $this->BandwidthBuyCategory->getAllBandwidthPackages();
        return view('bandwidth/item', $data); // Create a corresponding view
    }
    public function purchess()
    {
        ...
        return view('bandwidth/purchess', $data); // Create a corresponding view
    }
    ...
    // Method to show a specific bandwidth package by ID
    public function show($id)
    {
        $BandwidthBuyCategory = new BandwidthBuyCategory();
    ...
    public function item_category_store()
    {
        ...
        $model = new BandwidthBuyCategory();
    ...
    public function update($id)
    {
        ...
        // $this->bandwidthModel->updateBandwidthPackage($id, $data);
    ...
    {
        $data['providers'] = $this->vendorModel->getAllBandwidthPackages();
        return view('purchase/vendor', $data); // Create a corresponding view
    }
    public function add()
    {
...
public function vendor_save()
    ...
    public function purchess_fetch()
    {
        ...
        if (userHasPermission('customer', 'update')) {
            $datatables->addColumn('action', function ($row) {
                return '<div class="ipb-row-actions"><button type="button" class="ipb-row-btn tone-brand edit-bill" title="Edit bill"
            data-id="' . $row->id . '"
