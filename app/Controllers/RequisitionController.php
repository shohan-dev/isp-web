...
class RequisitionController extends BaseController
{
    public function index()
    {
        ...
        return view('purchase/requisition_list', [
            'title' => 'Requisition Bill',
            'units' => $unit,
            'items' => $allItems,
    ...
    public function getRequisition()
    {
        ...
        $requisitionRows = $requisitionModel->where('requisition_id', $id)->findAll();
    ...
    {
        $isApproved = $this->request->getPost('is_approved_mode');
        log_message('info', 'Is approved mode: ' . $isApproved);
        ...
        foreach ($items as $item) {
            ...
            if ($existing) {
                if ($isApproved == '1') {
                    ...
                    $modelR = new RequisitionModel();
                    $modelR->where('requisition_id', $requisitionId)
                        ->where('id', $item['id'])
                        ->delete();
                } else {
